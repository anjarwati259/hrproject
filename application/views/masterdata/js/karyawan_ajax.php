<script type="text/javascript">
  ...
  function save(){
    var data = new FormData(document.getElementById("form-karyawan"));
    for (const keyinput of data.keys()){
        let inputAwal = $("#form-karyawan").find('.is_invalid').remove();
    }

    $.ajax({
        type: 'POST',
        url: "<?php echo base_url('masterdata/karyawan/save'); ?>",
        data:data,
        dataType : 'json',
        processData: false,
        contentType: false,
        ...
        success: function(data){
            ...
            $.each(data.atribute, function (key, value){
                let formCheck = $("#form-karyawan #"+key);
  ...
  function edit(id){
    $.ajax({
        type: 'POST',
        url: "<?php echo base_url('masterdata/karyawan/getKaryawanById'); ?>",
        data:{id:id},
        dataType : 'json',
  ...
  function hapus(id){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'POST',
                    url: "<?php echo base_url('masterdata/karyawan/delete'); ?>",
                    data:{id:id},
                    dataType : 'json',
                    success: function(hasil) {
                      console.log(hasil)
  ...
  var dataTable = $('.datatables-basic').DataTable({  
     ...
     "order":[],  
     "ajax":{  
          url:"<?php echo base_url('masterdata/karyawan/getKaryawan'); ?>",  
          type:"POST"  
     },  
     "columnDefs":[  
          {  
